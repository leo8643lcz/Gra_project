...
#117 584
def cluster_matching_mask_g(target, mask_l,real_l,K,in_mask):
	...
	plt.imsave(target + "/mask_out_K" + str(K) + ".png", out_mask)

	return out_mask

def grab_cuting(target,K):
...
if __name__ == '__main__':
	...
	mask_dia,real_dia,real_data = img_dia(target,ori_mask,kernel,kernel2)
	...
	grab_mask = cluster_matching_mask_g(target,mask_l, real_l, 4, mask_full)
	#real = np.copy(real_data)
	grab_cuting(target, 4)
